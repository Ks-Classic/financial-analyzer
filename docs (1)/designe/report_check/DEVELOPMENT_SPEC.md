...
## 4. 画面構成
### 概要
主要な画面は単一のインタラクティブなページで構成され、PDFビューアと分析結果表示エリアが左右に配置される。PDFビューアは折りたたみ可能であり、分析結果エリアの視認性を高めることができる。
...
| メインエリア                                                      |
| +-------------------------+ +-----------------------------------+ |
| | ファイルアップロード    | | ローディング/エラー/完了メッセージ| |
| | (PDF選択、分析開始ボタン) | |                                   | |
| +-------------------------+ +-----------------------------------+ |
|                                                                   |
| +-------------------------+ +-----------------------------------+ |
| | PDFビューア               | | 分析結果リスト                    | |
| | (折りたたみボタン)        | | (ページタイトル、指摘事項)        | |
| | ページナビゲーション      | |                                   | |
| | 1ページ表示、スライド可   | |                                   | |
| +-------------------------+ +-----------------------------------+ |
+-------------------------------------------------------------------+
...
*   **PDFビューア**:
    *   アップロードされたPDFの各ページを1ページずつ表示する。
    *   「前へ」「次へ」ボタンでページをスライドできる。
    *   現在のページ番号と総ページ数を表示する。
    *   UI上部に折りたたみボタンを配置し、クリックすることでビューアを非表示にし、分析結果エリアを拡大表示できる。
    *   指摘箇所に将来的にハイライト表示を行う。
*   **分析結果リスト**:
    *   AIが指摘した問題点をリスト形式で表示する。
    *   各項目はページタイトル、ページ番号、指摘の種類、サマリー、詳細、関連テキスト（ハイライトテキスト）を含む。
    *   PDFビューアが非表示の際、UI上部に展開ボタンを配置し、クリックすることでビューアを再表示できる。
...
### FR2. PDF表示機能
*   アップロードされたPDFの内容をウェブUI上で表示できる。
*   PDFは1ページずつ表示され、ページ間をスライドして移動できる。
*   現在のページ番号と総ページ数が表示される。
*   PDFビューアは表示/非表示を切り替え可能で、非表示時は分析結果エリアが拡大される。
...
### FR4. 分析結果表示機能
*   AIによる分析結果をウェブUI上に一覧表示できる。
*   各指摘事項は、ページタイトル、ページ番号、指摘の種類、サマリー、詳細、ハイライトテキストを含む。
*   リスト項目をクリックすると、関連するPDFページに自動で移動する。
*   将来的に、指摘箇所のテキストがPDFビューア上でハイライト表示される。
